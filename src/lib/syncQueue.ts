...
// In-memory mutex to prevent concurrent sync races across app lifecycle events
let isSyncing = false;
...
export const syncQueue = {
  ...
  async addToQueue(type: QueueItem['type'], payload: any): Promise<void> {
    try {
      ...
      // Ensure client-generated idempotency UUID
      if (!enrichedPayload.id) {
        enrichedPayload.id = generateUUID();
      }
  ...
  async syncPendingQueue(onSyncSuccess?: (item: QueueItem) => void): Promise<{ success: boolean; syncedCount: number }> {
    if (isSyncing) {
      console.log('[SYNC MUTEX] syncPendingQueue already in progress. Skipping concurrent run.');
      ...
      for (const item of queue) {
        try {
          ...
          if (item.type === 'time_in' || item.type === 'time_out') {
            ...
            if (item.type === 'time_in') {
              const punchId = item.payload.id || item.id;
              const { error: err } = await supabase.from('time_logs').upsert({
                id: punchId,
                ...
                status: item.payload.status || 'verified',
                is_manual_entry: item.payload.is_manual_entry ?? false,
                photo_url: item.payload.photo_url || null,
                photo_status: item.payload.photo_status || null,
              }, { onConflict: 'id' });
              error = err;
            } else {
            ...
            const leaveId = item.payload.id || item.id;
            const { error: err } = await supabase.from('leaves').upsert({
              id: leaveId,
              technician_id: item.payload.technician_id,
            ...
            if (fetchErr) {
              ...
              if (updateErr) {
                ...
                if (txErr) {
                  ...
                  const commentId = item.payload.comment_id || generateUUID();
                  const { error: commentErr } = await supabase.from('ticket_comments').upsert({
                    id: commentId,
                    ticket_id: item.payload.ticket_id,
            ...
            const ticketId = item.payload.id || item.id;
            const { error: err } = await supabase.from('tickets').upsert({
              id: ticketId,
            ...
            const commentId = item.payload.id || item.id;
            const { error: err } = await supabase.from('ticket_comments').upsert({
              id: commentId,
              ticket_id: item.payload.ticket_id,
