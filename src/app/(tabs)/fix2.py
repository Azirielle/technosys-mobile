...
# Fix safeAlert and restore the missing states
original_part = '''  const safeAlert = (title: string, msg: string) => {
    if (Platform.OS === 'web') {
        .eq('employee_id', user.id)
        .order('created_at', { ascending: false });'''

fixed_part = '''  const safeAlert = (title: string, msg: string) => {
    if (Platform.OS === 'web') {
      window.alert(${title}\\n\\n);
    } else {
...
content = content.replace(original_part, fixed_part)
